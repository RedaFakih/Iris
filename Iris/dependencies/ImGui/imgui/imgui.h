...
namespace ImGui
{
    ...
    IMGUI_API void          ShowStyleEditor(ImGuiStyle* ref = NULL, int* outThemeID = nullptr);    // add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
    ...
    IMGUI_API bool          ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& uv0 = ImVec2(0, 0),  const ImVec2& uv1 = ImVec2(1,1), const ImVec4& bg_col = ImVec4(0,0,0,0), const ImVec4& tint_col = ImVec4(1,1,1,1));    // <0 frame_padding uses default frame padding settings. 0 for no padding
    ...
    IMGUI_API bool          TreeNodeEx(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt = nullptr, ...) IM_FMTARGS(3);
    ...
    IMGUI_API void          TableHeader(const char* label, ImU32 textColor = 0, ImU32 textColHov = 0);    // submit one header cell manually (rarely used)

...
namespace Iris {
	...
	public:
		...
		virtual bool IsAssetHandleValid(AssetHandle handle) = 0; // Asset Handle being valid... (Says nothing about the asset itself)
		virtual bool IsMemoryAsset(AssetHandle handle) = 0; // Asset exists in memory only, there is no file backing it

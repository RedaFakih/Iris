...
namespace Iris {
	...
	std::string MaterialAssetSerializer::SerializeToYAML(Ref<MaterialAsset> materialAsset) const
	{
		...
		out << YAML::Key << "Material" << YAML::Value;
		{
			...
			bool shadowCasting = materialAsset->IsShadowCasting();
			out << YAML::Key << "Transparent" << YAML::Value << transparent;
			out << YAML::Key << "TwoSided" << YAML::Value << twoSided;
			out << YAML::Key << "ShadowCasting" << YAML::Value << shadowCasting;
	...
	bool MaterialAssetSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<MaterialAsset>& targetAsset, AssetHandle handle) const
	{
		...
		bool shadowCasting = materialNode["ShadowCasting"].as<bool>(true);
		...
		targetAsset->SetShadowCasting(shadowCasting);
		...
		if (materialNode["MaterialFlags"].as<uint32_t>(0) != targetAsset->GetMaterial()->GetMaterialFlags())
			IR_CORE_ERROR_TAG("Mesh", "Error when loading Iris Material file: Serialized material flags bit field is not equal to the one generated by serialized properties.");

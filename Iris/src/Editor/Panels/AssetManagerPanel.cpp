	...
	void AssetManagerPanel::OnImGuiRender(bool& open)
	{
		if (ImGui::Begin("Asset Manager", &open, ImGuiWindowFlags_NoCollapse))
		{
			...
			if (ImGui::TreeNodeEx("Registry", treeNodeFlags))
			{
				...
				static const char* filterOptions[9] = { "None", "Texture", "StaticMesh", "MeshSource", "MeshCollider", "Material", "Scene", "Font", "EnvironmentMap" };
				static int selectedIndex = 0;
				UI::PropertyDropdown("Type Filter", filterOptions, 9, &selectedIndex, "Set a filter for the asset types to display");

...
namespace Iris::Utils {
	...
	inline void VulkanCheckResult(VkResult res)
	{
		if (res != VK_SUCCESS)
		{
			IR_CORE_ERROR_TAG("Vulkan", "VkResult is `{0}`", VKResultToString(res));
			...
			IR_ASSERT(res == VK_SUCCESS, "Device lost!");
		}
	}
	...
	inline void VulkanCheckResult(VkResult res, const char* file, int line)
	{
		if (res != VK_SUCCESS)
		{
			IR_CORE_ERROR_TAG("Vulkan", "VkResult is `{0}` in file: `{1}` at line: `{2}`", VKResultToString(res), file, line);
			if (res == VK_ERROR_DEVICE_LOST)
			{
				using namespace std::chrono_literals;
				std::this_thread::sleep_for(3s);
...
{\
	VkResult res = (f);\
	::Iris::Utils::VulkanCheckResult(res, __FILE__, __LINE__);\
}

namespace Iris::VKUtils {

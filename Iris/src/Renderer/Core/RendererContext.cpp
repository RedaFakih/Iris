   ...
   static  bool s_Validation = true;
    ...
    namespace Utils {
        ...
        static bool CheckDriverAPIVersionSupport(uint32_t minimumSupportedVersion)
        {
            ...
            {
                IR_CORE_FATAL_TAG("Renderer", "Incompatible driver version!");
                IR_CORE_FATAL_TAG("Renderer", "\tYou have: {0}.{1}.{2}", VK_API_VERSION_MAJOR(instanceVersion), VK_API_VERSION_MINOR(instanceVersion), VK_API_VERSION_PATCH(instanceVersion));
                IR_CORE_FATAL_TAG("Renderer", "\tYou need at least: {0}.{1}.{2}", VK_API_VERSION_MAJOR(minimumSupportedVersion), VK_API_VERSION_MINOR(minimumSupportedVersion), VK_API_VERSION_PATCH(minimumSupportedVersion));
            ...
            void* pUserData)
        {
            ...
            if (labels.size())
            {
                if (messageType & VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT)
                    IR_CORE_WARN_TAG("Renderer", "VulkanValidation: Type: {0}, Severity: {1}\nMessage: \n{2}\n{3}, {4}",
                    ...
                    IR_CORE_ERROR_TAG("Renderer", "VulkanValidation: Type: {0}, Severity: {1}\nMessage: \n{2}\n{3}, {4}",
                        VkDebugUtilsMessageType(messageType),
                        VkDebugUtilsMessageSeverity(messageSeverity),
                        pCallbackData->pMessage,
                        labels,
    ...
    RendererContext::~RendererContext()
    {
        if (s_Validation)
        {
            ...
            IR_VERIFY(vkDestroyDebugUtilsMessengerEXT != nullptr, "vkGetInstanceProcAddr returned null!");
    ...
    {
        IR_VERIFY(glfwVulkanSupported(), "Vulkan is not supported with this GLFW!");

        if (!Utils::CheckDriverAPIVersionSupport(VK_API_VERSION_1_2))
        {
            IR_VERIFY(false, "Incompatible vulkan driver!");
        }
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_win32_surface"
        ...
        if (s_Validation)
        {
            ...
            IR_CORE_INFO_TAG("Renderer", "Vulkan instance layers:");
            for (const VkLayerProperties& layer : instanceLayerProperties)
            {
                IR_CORE_INFO_TAG("Renderer", "\t{0}", layer.layerName);
            ...
            {
                IR_CORE_ERROR_TAG("Renderer", "Validation layer VK_LAYER_KHRONOS_validation not present, validation is disabled!");
            }
        }
        ...
        if (s_Validation)
        {
            ...
            IR_ASSERT(vkCreateDebugUtilsMessengerEXT != nullptr, "vkGetInstanceProcAddr returned null!");

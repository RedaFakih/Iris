...
namespace Iris {
	...
	struct FramebufferSpecification
	{
		...
		// NOTE: This is not generally needed since the scale of framebuffers is just set by the global scene renderer
	...
	public:
		...
		Ref<Texture2D> GetResolveImage(uint32_t attachmentIndex = 0) const { IR_ASSERT(attachmentIndex < m_ColorResolveImages.size()); return m_ColorResolveImages[attachmentIndex]; }

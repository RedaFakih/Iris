...
namespace Iris {
	...
	std::map<VkShaderStageFlagBits, std::string> ShaderPreProcessor::PreprocessShader(const std::string& source) const
	{
		...
		IR_VERIFY(shaderSource.size(), "Shader has no code...");
		...
		IR_VERIFY((tokens.size() >= 3) && (tokens[1] == "version"), "Invalid `#version` expression or not found!");
		...
		while (pos != std::string::npos)
		{
			...
			if (tokens[index] == "stage")
			{
				...
				IR_VERIFY(stage == "vertex" || stage == "fragment" || stage == "compute", "Invalid shader type/stage specified!");
			...
			// 	{
			// 		++index;
			// 		IR_VERIFY(tokens[index] == "(", "whatever");
			// 		++index;
		...
		IR_VERIFY(stagePositions.size(), "Could not preprocess the shader since there are no recognized stages in the file!");

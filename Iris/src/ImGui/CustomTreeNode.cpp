...
namespace ImGui {
	...
	bool TreeNodeWithIcon(Iris::Ref<Iris::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint)
	{
		...
		if (display_frame)
		{
			...
			if (icon)
			{
				...
				Iris::UI::ShiftCursorY(-frame_height + pad);
				Iris::UI::ShiftCursorX(arrowWidth);
				Iris::UI::Image(icon, { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
	...
	bool TreeNodeWithIcon(Iris::Ref<Iris::Texture2D> icon, const char* label, ImGuiTreeNodeFlags flags, ImColor iconTint)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return false;

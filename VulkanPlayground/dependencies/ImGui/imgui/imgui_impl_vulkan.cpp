...
static void CreateOrResizeBuffer(VkBuffer& buffer, VmaAllocation& buffer_allocation, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    vkPlayground::VulkanAllocator allocator("ImGuiAllocator");

    if (buffer != VK_NULL_HANDLE && buffer_allocation != nullptr)
        allocator.DestroyBuffer(buffer_allocation, buffer);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        vtx_dst = allocator.MapMemory<ImDrawVert>(rb->VertexBufferAllocation);
        idx_dst = allocator.MapMemory<ImDrawIdx>(rb->IndexBufferAllocation);
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Create the Image:
    {
        ...
        bd->FontAllocation = allocator.AllocateImage(&info, VMA_MEMORY_USAGE_GPU_ONLY, &bd->FontImage);
    }
    ...
    // Create the Upload Buffer:
    {
        ...
        buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        bd->UploadBufferAllocation = allocator.AllocateBuffer(&buffer_info, VMA_MEMORY_USAGE_CPU_TO_GPU, &bd->UploadBuffer);
    ...
    // Upload to Buffer:
    {
        char* map = allocator.MapMemory<char>(bd->UploadBufferAllocation);
        memcpy(map, pixels, upload_size);
        allocator.UnmapMemory(bd->UploadBufferAllocation);
    }
...
void    ImGui_ImplVulkan_DestroyFontUploadObjects()
{
    ...
    if (bd->UploadBuffer != VK_NULL_HANDLE && bd->UploadBufferAllocation != nullptr)
    {
        allocator.DestroyBuffer(bd->UploadBufferAllocation, bd->UploadBuffer);
        bd->UploadBuffer = VK_NULL_HANDLE;
        bd->UploadBufferAllocation = nullptr;
    }
}
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->FontImage != VK_NULL_HANDLE && bd->FontAllocation != nullptr)
    {
        allocator.DestroyImage(bd->FontAllocation, bd->FontImage);
        bd->FontImage = VK_NULL_HANDLE;
        bd->FontAllocation = nullptr;
    }
...
ImTextureID ImGui_ImplVulkan_AddTexture_Internal(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ...
    VkResult err = vkAllocateDescriptorSets(v->Device, &alloc_info, &desc_set);
    check_vk_result(err);

    ImGui_ImplVulkan_UpdateTextureInfo(desc_set, sampler, image_view, image_layout);
    return (ImTextureID)desc_set;
}
...
void ImGui_ImplVulkanH_DestroyFrameRenderBuffers(VkDevice device, ImGui_ImplVulkanH_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    vkPlayground::VulkanAllocator vkPGAllocator("ImGuiAllocator");

    if (buffers->VertexBuffer != VK_NULL_HANDLE && buffers->VertexBufferAllocation != nullptr)
    {
        vkPGAllocator.DestroyBuffer(buffers->VertexBufferAllocation, buffers->VertexBuffer);
        buffers->VertexBuffer = VK_NULL_HANDLE;
        buffers->VertexBufferAllocation = nullptr;
    }

    if (buffers->IndexBuffer != VK_NULL_HANDLE && buffers->IndexBufferAllocation != nullptr)
    {
        vkPGAllocator.DestroyBuffer(buffers->IndexBufferAllocation, buffers->IndexBuffer);

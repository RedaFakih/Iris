...
namespace vkPlayground {
	...
	Ref<MeshSource> AssimpMeshImporter::ImportToMeshSource()
	{
		...
		VKPG_CORE_INFO_TAG("Mesh", "Loading mesh: {0}", m_AssetPath);
		...
		if (!scene)
		{
			VKPG_CORE_ERROR_TAG("Mesh", "Failed to load mesh file: {0}", m_AssetPath);
			return nullptr;
		}
		...
		if (scene->HasMeshes())
		{
			...
			for (uint32_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				{
					VKPG_CORE_ERROR_TAG("Mesh", "Mesh index {0} with name `{1}` has no vertex positions... skipping import!", m, mesh->mName.C_Str());
				}
				if (!mesh->HasNormals())
				{
					VKPG_CORE_ERROR_TAG("Mesh", "Mesh index {0} with name `{1}` has no vertex normals, and they could not be computed... skipping import!", m, mesh->mName.C_Str());
				}
				...
				{
					VKPG_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices since we are using aiProcess_Triangulate");
		...
		{
			VKPG_CORE_TRACE_TAG("Mesh", "----- Materials - {0} -----", m_AssetPath);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				VKPG_CORE_TRACE_TAG("Mesh", "\t   {0} (index = {1})", aiMaterialName.data, i);
				...
				VKPG_CORE_TRACE_TAG("Mesh", "\t   COLOR = {0} - {1} - {2}", aiColor.r, aiColor.g, aiColor.b);
				VKPG_CORE_TRACE_TAG("Mesh", "\t   ROUGHNESS = {0}", roughness);
				VKPG_CORE_TRACE_TAG("Mesh", "\t   METALNESS = {0}", metalness);
				...
				if (hasAlbedoMap)
				{
					...
					else
					{
						...
						VKPG_CORE_TRACE_TAG("Mesh", "\t   Albedo map path: {0}", texturePath);
					...
					{
						fallback = true;
						VKPG_CORE_ERROR_TAG("Mesh", "\t   Could not load texture: {0}", aiTexPath.C_Str());
					}
				}

				if (fallback)
				{
					VKPG_CORE_TRACE_TAG("Mesh", "\t   No albedo map");
				...
				if (hasNormalMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath(parentPath.string());
						VKPG_CORE_TRACE_TAG("Mesh", "\t   Normal map path: {0}", texturePath);
				...
				if (fallback)
				{
					VKPG_CORE_TRACE_TAG("Mesh", "\t   No normal map");
				...
				if (hasRoughnessMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						VKPG_CORE_TRACE_TAG("Mesh", "\t   Roughness map path: {0}", texturePath);
				...
				if (fallback)
				{
					VKPG_CORE_TRACE_TAG("Mesh", "\t   No roughness map");
				...
				if (hasMetalnessMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						VKPG_CORE_TRACE_TAG("Mesh", "\t   Metalness map path: {0}", texturePath);

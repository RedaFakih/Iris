...
namespace vkPlayground {
	...
	public:
		...
		void Set(std::string_view name, Ref<Texture2D> value);
		void Set(std::string_view name, Ref<Texture2D> value, uint32_t arrayIndex);
		...
		VkDescriptorSet GetDescriptorSet(uint32_t frame)
		{
			...
			Prepare();
			return m_DescriptorSetManager.GetDescriptorSets(frame)[0]; // 3 since that is the material specific set
		}

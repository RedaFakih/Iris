...
namespace vkPlayground {
	...
	// TODO: This class is still just shit and does not work for the runtime since in the runtime it will not allocate any command buffer
	// so the m_CommandBuffers.size() will be = 0 and we can NOT do % 0 so it will always fail for the runtime layer so we need to find a way around
	// this problem (Suggested fixes to think about: Use flags to know in what application state we are {runtime, editor})
